...
def check_passwd(password: str) -> (bool,):
    errors = []
    if (len(password) < 8):
        errors.append(f"Password is too short ({len(password)} symbols). Should be 8 or more symbols")
    if (password.islower()):
        errors.append("No capitalized symbols. Password should contains at least 1 capitalized character")
